...
onMounted(async () => {
  try {
    ...
    runtimeWarmupWarmedInCurrentSession = prefetched?.data?.warmed === true;
...
onUnmounted(() => {
  ...
  if (disconnectSaveTimer) clearTimeout(disconnectSaveTimer);
  if (roomIdSaveTimer) clearTimeout(roomIdSaveTimer);
  if (warmupConfirmResolver) {
    warmupConfirmResolver(false);
    warmupConfirmResolver = null;
  }
  offLoginStatus?.();
});
...
async function handleConnect() {
  ...
  try {
    ...
    runtimeWarmupWarmedInCurrentSession = true;
  } catch (e: any) {
...
function resolveWarmupConfirm(confirmed: boolean) {
  showWarmupConfirmModal.value = false;
  const resolve = warmupConfirmResolver;
  warmupConfirmResolver = null;
  resolve?.(confirmed);
}
...
<template>
  <div class="page">
    ...
    <!-- 首次连接预载确认弹窗（主题化，替代系统 window.confirm） -->
    <Teleport to="body">
      <div v-if="showWarmupConfirmModal" class="modal-overlay" @click.self="cancelWarmupConfirm">
        <div class="modal-content warmup-confirm-modal" role="dialog" aria-modal="true" aria-labelledby="warmup-confirm-title">
          <div class="modal-header">
            <h3 id="warmup-confirm-title" class="modal-title">首次连接提示</h3>
            <button class="modal-close" @click="cancelWarmupConfirm">&times;</button>
          </div>
          <div class="modal-body">
