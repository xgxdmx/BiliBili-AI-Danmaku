...
| 版本 | 日期 | 主要改动                                                                                                                                                                                                      |
|:------:|:--------:|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **v0.4.0** | 2025-04 | Python 入口 `run.py` → `danmaku.py` 全链路重命名（含 PyInstaller / 构建 / 部署脚本 / 安装器）• 关于页面新增关闭按钮行为设置（询问/最小化到托盘/直接退出）• 主进程可读性重构（弹幕路由纯函数、配置 IPC 助手拆分）• AI 握手兼容增强（多个 fallback 提取路径 + 响应预览诊断）• 样式外提收尾（所有页面内联样式迁移至独立 CSS） |
